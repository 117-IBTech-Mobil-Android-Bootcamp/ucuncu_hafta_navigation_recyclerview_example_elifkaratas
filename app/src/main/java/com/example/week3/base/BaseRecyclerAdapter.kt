...
abstract class BaseRecyclerAdapter<T ,VH : RecyclerView.ViewHolder>(private val mList : List<T>, private val block : (T) -> Unit = { }) : RecyclerView.Adapter<VH>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH =  getViewHolder(parent)
    override fun onBindViewHolder(holder: VH, position: Int) { bind(mList[position], holder) }
    ...
    override fun getItemCount(): Int = this.mList.size
     ...
     open inner class BaseViewHolder(val rootView : View, block: (T) -> Unit) : RecyclerView.ViewHolder(rootView){
